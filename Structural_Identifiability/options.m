...
%%% (1) MODEL NAME: 
modelname = 'Model_SubMIC_Carvacrol';  
...
opts.checkObsIn  = 0;       % check input observability (1 = yes; 0 = no).
...
opts.maxLietime  = 1e6;     % max. time allowed for calculating 1 Lie derivative.
...
opts.nnzDerW     = 0;       % numbers of nonzero derivatives of the unmeasured inputs (w); may be 'inf'
...
opts.affine_tStage       = 0;     % max. computation time for the last iteration.
opts.affine_kmax         = 0;     % max. number of iterations.
opts.affine_parallel     = 0;     % use parallel toolbox (=1) or not (=0) to calculate partial ranks.
opts.affine_workers      = 0;     % number of workers for parallel pool.
opts.affine_graphics     = 0;     % display graphics (=1) or not (=0)
opts.affine_delete_model = 0;     % delete affine model when finished (=1) or not (=0).
...
opts.results_file = []; % .mat file to use if use_existing_results = 1                              
