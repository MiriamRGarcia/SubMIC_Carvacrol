### Generate figures for cell number subject to Carvacrol therapy:
...
mu_g0 <- c(2.7993,2.7233)
mu_d0 <- c(8.6042e-15,3.5874e-02)
k_S   <- c(1.6207e-01,1.5092e-01)
k_g   <- c(4.7341e-04,1.5645e-03) 
k_d   <- c(1.4653e-01,5.6927e-04)
...
plot(t, sol_Bc[1,], type = "b", col = cc[1], xlab = "t [h]", ylab = "CFUs/mL", main="B. Cereus")

lines(t_exp, Exp1_CFUs_Bc[1,], type="p", col = cc[1])
lines(t_exp, Exp2_CFUs_Bc[1,], type="p", col = cc[1])

for(i in 2:n_exp){
  lines(t,sol_Bc[i,], type = "o",col = cc[i])
  lines(t_exp,Exp1_CFUs_Bc[i,], type = "p",col = cc[i])
  lines(t_exp,Exp2_CFUs_Bc[i,], type = "p",col = cc[i])
}

# (4.3) Plot experimental data VS model for E. Coli:

plot(t, sol_Ec[1,], type = "b", col = cc[1], xlab = "t [h]", ylab = "CFUs/mL", main="E. Coli")

lines(t_exp, Exp1_CFUs_Ec[1,], type="p", col = cc[1])
lines(t_exp, Exp2_CFUs_Ec[1,], type="p", col = cc[1])

for(i in 2:n_exp){
  lines(t, sol_Ec[i,], type = "o", col = cc[i])
  lines(t_exp, Exp1_CFUs_Ec[i,], type = "p", col = cc[i])
  lines(t_exp, Exp2_CFUs_Ec[i,], type = "p", col = cc[i])
}
